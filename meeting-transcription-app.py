...
"""
    ...
    def generate_pdf_report(self, meeting_info, summary, insights, action_items, takeaways):
        ...
        pdf.multi_cell(0, 5, f"Title: {meeting_info.get('title', 'N/A')}\n"
                              f"Date: {meeting_info.get('date', 'N/A')}\n"
                              f"Time: {meeting_info.get('time', 'N/A')}\n"
                              f"Location: {meeting_info.get('location', 'N/A')}\n"
                              f"Organizer: {meeting_info.get('organizer', 'N/A')}\n"
                              f"Attendees: {meeting_info.get('attendees', 'N/A')}")
        pdf.ln(5)
        ...
        if takeaways:
            ...
            pdf.set_font("Arial", "", 10)
            for i, takeaway in enumerate(takeaways, 1):
                pdf.multi_cell(0, 5, f"{i}. {takeaway}")
            pdf.ln(5)
...
def main():
    ...
    lang = st.sidebar.selectbox(
        "🌍 " + t("language", "en"),
        options=["en", "es", "fr", "zh", "de"],
        format_func=lambda x: {"en": "English", "es": "Español", "fr": "Français", "zh": "中文", "de": "Deutsch"}[x]
    )
